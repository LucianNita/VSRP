...
@dataclass(slots=True)
class FleetUncertaintyScenario:
    """
    One realized uncertainty scenario for a fleet episode.

    Each vessel receives its own UncertaintyScenario drawn independently,
    capturing vessel-specific initial delays, port handling multipliers,
    and weather speed factors. Carbon and fuel prices are shared market
    variables common to all vessels in the episode.

    Parameters
    ----------
    vessel_scenarios : list[UncertaintyScenario]
        One realized scenario per vessel, in vessel order.
        vessel_scenarios[v].realized_initial_delay_h is vessel v's delay.
    realized_carbon_price_eur : float
        Shared realized ETS carbon price for the episode.
    realized_fuel_price_usd : float
        Shared realized fuel price for the episode.
    """
    vessel_scenarios: list[UncertaintyScenario]
    realized_carbon_price_eur: float
    realized_fuel_price_usd: float
    ...
    def n_vessels(self) -> int:
        """Number of vessel scenarios in this fleet scenario."""
        return len(self.vessel_scenarios)

    def vessel_scenario(self, vessel_idx: int) -> UncertaintyScenario:
        """Return the UncertaintyScenario for vessel at position vessel_idx."""
        return self.vessel_scenarios[vessel_idx]


def sample_fleet_uncertainty_scenario(
    rng: np.random.Generator,
    fleet_instance,
    config: UncertaintyConfig,
) -> FleetUncertaintyScenario:
    """
    Sample one realized fleet uncertainty scenario.

    Each vessel receives an independently drawn UncertaintyScenario.
    Carbon and fuel prices are drawn once and shared across all vessels,
    reflecting that they are market-level variables rather than
    vessel-specific operational variables.

    Parameters
    ----------
    rng : np.random.Generator
        Seeded RNG for reproducibility.
    fleet_instance : FleetInstance
        Used to determine the number of vessels and valid port indices.
    config : UncertaintyConfig
        Distribution parameters for all uncertainty components.

    Returns
    -------
    FleetUncertaintyScenario
        One realized stochastic fleet scenario.
    """
    realized_carbon_price_eur = _clip_normal(
        rng,
        config.carbon_price_mean_eur,
    ...
    vessel_scenarios: list[UncertaintyScenario] = []
    for vessel_instance in fleet_instance.vessel_instances:
        vessel_scenario = sample_uncertainty_scenario(
            rng,
            vessel_instance,
            config,
        )
        vessel_scenario = UncertaintyScenario(
            realized_initial_delay_h=vessel_scenario.realized_initial_delay_h,
            port_handling_multipliers=vessel_scenario.port_handling_multipliers,
            weather_speed_factors=vessel_scenario.weather_speed_factors,
            realized_carbon_price_eur=realized_carbon_price_eur,
            realized_fuel_price_usd=realized_fuel_price_usd,
        )
        vessel_scenarios.append(vessel_scenario)

    return FleetUncertaintyScenario(
        vessel_scenarios=vessel_scenarios,
        realized_carbon_price_eur=realized_carbon_price_eur,
        realized_fuel_price_usd=realized_fuel_price_usd,
    )


def simulate_fleet_realized_performance(
    fleet_instance,
    fleet_solution,
    fleet_scenario: FleetUncertaintyScenario,
) -> list[RealizedPerformance]:
    """
    Simulate realized performance for all vessels in a fleet episode.

    Each vessel is evaluated independently under its own vessel scenario.
    The results are returned as a list of RealizedPerformance objects
    in vessel order.

    Parameters
    ----------
    fleet_instance : FleetInstance
        Fleet-level instance with one VSRPInstance per vessel.
    fleet_solution : FleetSolution
        Fleet-level solution with one VSRPSolution per vessel.
    fleet_scenario : FleetUncertaintyScenario
        Realized fleet scenario with one UncertaintyScenario per vessel.

    Returns
    -------
    list[RealizedPerformance]
        One RealizedPerformance per vessel, in vessel order.
    """
    results: list[RealizedPerformance] = []

    for vessel_instance, vessel_solution, vessel_scenario in zip(
        fleet_instance.vessel_instances,
        fleet_solution.vessel_solutions,
        fleet_scenario.vessel_scenarios,
    ):
